...
import { createClient, PlaceDetailsResult, PlaceSearchResult, AddressComponent } from '@google/maps';
...
export async function lookupPlace(placeID: string): Promise<PlaceDetailsResult> {
...
export function findCountry(addressComponents: AddressComponent[]): string {
	const countryComponent = addressComponents.find(comp => comp.types.indexOf('country') > -1);
	if (!countryComponent) {
...
export async function lookupPhoto(obj: PlaceSearchResult): Promise<string | null> {
