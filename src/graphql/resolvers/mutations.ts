...
export default {
    async createSuite(parent, args: CreateSuiteArgs, context: ResolverContext) {
        ...
        await db.PersonSuites.insert({
            person: user,
            suite,
            role: PersonSuiteRole.OWNER
        });
    ...
    async inviteOwner(
        parent,
        { suiteID, phoneNumber }: InviteOwnerArgs,
        { user, db }: ResolverContext,
        info
    ) {
        const suite = await db.Suites.findOneOrFail({
            where: { nodeID: suiteID },
            relations: ['buzzer']
        });
        ...
        await db.PersonSuites.insert({
            ...
            suite,
            role: PersonSuiteRole.OWNER,
            hashedUnlockCode: hash
        });
        const newSuite = await db.Suites.findOneOrFail(suite.id, {
    ...
    async unlinkPerson(
        ...
        const toDelete = await db.PersonSuites.findOneOrFail({
            where: { person, suite }
        });
    ...
    async deleteSuite(
        parent,
        args: { [suiteID: string]: any },
        context: ResolverContext,
        info
    ) {
        const { db } = context;
        const suite = await db.Suites.findOne({
            where: { nodeID: args.suiteID }
        });
        if (!suite) {
